...
class MainView extends React.Component {
    ...
    loadTickets = () => {
        ...
        $.ajax(`/api/ticket/accounts/list?q=${this.state.search}&from_date=${this.state.from_date}&to_date=${this.state.to_date}&place=${this.state.place}&state=${this.state.state}&city=${this.state.city}&page=${this.state.page}&tower=${this.state.tower}&sector=${this.state.sector}&problem=${this.state.problem}&solution=${this.state.solution}&fixed_by=${this.state.fixer}&sort_by=${this.state.sort}`, {
            success: (res) => {
                console.log("--------------------");
                ...
                console.log("--------------------");
                let items = [...this.state.tickets, ...res["data"]];
    ...
    render() {
        ...
        return (
            <div className="row">
                ...
                <div className="w-100 bg-hti rounded-pill">
                    <div className="row">
                        <div className="row">
                            ...
                            <div className="col-2">الباقة</div>
                            <div className="col-2">تاريخ نهاية الاشتراك</div>
                            <div className="col-2">الهاتف</div>
                            <div className="col-1">الحالة</div>
                ...
                <div className="col-12 my-2">
                    {this.state.tickets.map((ticket, idx) =>
                        <div className="row px-2" id="ticket-">
                            <div className={`bg-light ${ticket.close_state == "1" ? "text-success" : ""} ${ticket.close_state == "2" ? "text-danger" : ""} ${ticket.close_state == "3" ? "text-info" : ""} shadow-none mt-1 rounded py-2`}>
                                <div className="row">
                                    <div id="ticket-id-" className="col">
                                        <div>{ticket.firstname} {ticket.lastname}</div>
                                        <div> {ticket.username}</div>
                                    </div>
                                    <div className="col-2">
                                        <div id="ticket-account-"> {ticket.profile_details? ticket.profile_details.name : "Null" } </div>
                                        
                                    </div>
                                    <div id="ticket-phone-" className="col-2"> {ticket.expiration}</div>
                                    <div id="ticket-phone-" className="col-1"> {ticket.phone}</div>
                                    <div id="ticket-place-container-" className="col-2 d-none d-md-block">
                                    ...
                                    <div className="col-1 d-none d-md-block p-0 m-0">
                                        ...
                                        {/* {% endif %} */}
                                        {/* {% if "Ticket.can_edit_ticket" in perms %} */}
                ...
                {this.state.reportExporter != null ? <ProblemsReportView hide={() => this.setState({ delete: null })} /> : ""}
